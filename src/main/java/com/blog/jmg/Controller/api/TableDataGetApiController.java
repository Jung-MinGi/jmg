...
//        });
    ...
    public TempImg TmpimageSaveToS3(MultipartFile file) throws IOException {
        log.error("AmazonS3Client={}",amazonS3Client);
        log.error("bucketName={}",bucketName);
        log.error("tempPath={}",tempPath);
    ...
    public ResponseEntity<Object> summerAll(@RequestBody String contents) throws IOException {
        ...
        for (String s : map.keySet()) {
            if (!pattern.matcher(map.get("title")).matches())
                return new ResponseEntity<>("제목에 특수문자 사용 금지", HttpStatus.BAD_REQUEST);
